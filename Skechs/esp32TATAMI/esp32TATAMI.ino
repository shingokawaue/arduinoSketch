...
IPAddress ip(192, 168, 0, 150);           // for fixed IP Address
...
void setup() {
 ...
 //DS3231 RTC リアルタイムクロックモジュール----------------------------------------------------------------
  ...
  display.display();
  //rtc.adjust( DateTime(2019, 5,26, 9, 23, 30)  );
  display.println("TC!");
  display.display();
...
void playP() {
  for(int i = 0;i < 8; i++){
  digitalWrite(P_BUZZER_PIN , HIGH);
  delay(300);
  digitalWrite(P_BUZZER_PIN , LOW);
  delay(300);
  }
}
void play() {
